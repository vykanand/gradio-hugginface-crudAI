...
(function () {
  function getValueFromPayload(payload, fieldPath) {
    ...
    if (parts.length === 1) {
      ...
      if (payload && payload.detail && Object.prototype.hasOwnProperty.call(payload.detail, key)) return payload.detail[key];
      if (payload && payload.data && Object.prototype.hasOwnProperty.call(payload.data, key)) return payload.data[key];
    }

    // Try nested inside detail
    v = payload && payload.detail ? payload.detail : undefined;
    if (v) {
      let tmp = v;
      for (const p of parts) {
        if (tmp && Object.prototype.hasOwnProperty.call(tmp, p)) tmp = tmp[p];
        else {
          tmp = undefined;
          break;
        }
