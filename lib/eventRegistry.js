...
(function () {
  ...
  // Use path.resolve() to properly normalize the path on Windows
  const DB_PATH = process.env.EVENT_REGISTRY_DB || path.resolve(path.join(__dirname, '..', 'storage', 'event_registry_db'));
    ...
    try {
      ...
      try {
        if (level && !db) {
          ...
          db.once('error', (err) => {
            console.warn('[eventRegistry] level db error:', err && err.message ? err.message : err);
            db = null;
          });
        }
        if (db) {
        ...
        console.warn('[eventRegistry] db operation failed:', e && e.message ? e.message : e);
      }
  ...
  function init(opts) {
    try {
      // Use shared DB from eventBus if provided, otherwise lazy-init on demand
      // This prevents LOCK conflicts when both eventRegistry and eventBus try to open the same DB
      if (opts && opts.sharedDB) {
        db = opts.sharedDB;
        console.log('[eventRegistry] Using shared DB from eventBus');
      }
  ...
  async function reconcile() {
    ...
    try {
      ...
      // Do NOT open DB here - eventBus already has it open
      // Opening the same LevelDB instance twice causes LOCK errors
      // This reconcile function is called from eventBus which manages the DB
      if (!db) {
        // DB not available yet - skip reconciliation for now
        // It will be opened lazily during register() when persistence is needed
        return { ok: false, reason: 'db-not-initialized-yet' };
      }
      const seen = new Set();
